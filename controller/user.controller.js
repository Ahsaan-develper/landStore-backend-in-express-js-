...
import { blacklistHeaderTokens } from "../utils/tokenBlackList.js";
...
const headerAccess = req.headers.authorization?.replace('Bearer ', '');
        const headerRefresh = req.headers['x-refresh-token']?.replace('Bearer ', '');
        ...
        if (headerAccess || headerRefresh) {
            blacklistHeaderTokens(headerAccess, headerRefresh);
        }
